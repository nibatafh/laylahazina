...
* `hl` (default `en` for English) the [language code](https://github.com/danieliu/play-scraper/blob/master/laylahazina/constants.py#L1) to receive results in a specific language
    * Note: non-default will result in certain fields as `None` due to the way an app's additional info section HTML is displayed
* `gl` (default `us` for United States) the [country code](https://github.com/danieliu/play-scraper/blob/master/laylahazina/constants.py#L87) to receive results based from a specific country

```python
>>> import laylahazina
>>> print(laylahazina.details('com.android.chrome'))
{
...
* `collection` a [collection](https://github.com/danieliu/play-scraper/blob/master/laylahazina/lists.py#L3) to fetch.
* `category` (default None) a [category](https://github.com/danieliu/play-scraper/blob/master/laylahazina/lists.py#L12) to filter by.
* `results` (default 60, max 120) the number of apps to fetch.
* `page` (default 0) the page number to fetch. Limit: `page * results <= 500`.
* `age` (default None) an [age range](https://github.com/danieliu/play-scraper/blob/master/laylahazina/lists.py#L74) to filter by. (Only for FAMILY categories)
...
```python
>>> import laylahazina
>>> print(laylahazina.developer('Disney', results=5))
...
```python
>>> import laylahazina
>>> print(laylahazina.suggestions('cat'))
...
```python
>>> import laylahazina
>>> print(laylahazina.search('dogs', page=2))
...
```python
>>> import laylahazina
>>> print(laylahazina.similar('com.supercell.clashofclans', results=5))
